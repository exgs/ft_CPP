...
ClapTrap::ClapTrap(
		...
		_max_energy_points(_max_energy_points),
		_level(_level),
		_melee_attack_damage(_melee_attack_damage),
		_ranged_attack_damage(_ranged_attack_damage),
...
ClapTrap::ClapTrap(const ClapTrap &ref)
{
	...
	_name = ref._name;
	std::cout << "[ClapTrap] : " << "<" << ref._name << ">" << "복사!" << std::endl;
}
...
ClapTrap& ClapTrap::operator=(const ClapTrap &ref)
{
	...
	_ranged_attack_damage = ref._ranged_attack_damage;
	_armor_damage_reduction = ref._armor_damage_reduction;
	std::cout << "[ClapTrap] : " << "<" << ref._name << ">" << "대입!" << std::endl;
	return (*this);
}
...
{
	if (value >= MINUS) // 음수를 넣어준 경우
		value = 0;
	_armor_damage_reduction = value;
}

unsigned int	ClapTrap::gettArmor()
{
